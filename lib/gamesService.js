...
export const gamesService = {
  loadGames: withCache(_gamesService.loadGames, 5 * 60 * 1000), // 5 دقائق
  loadGame: withCache(_gamesService.loadGame, 10 * 60 * 1000), // 10 دقائق
  loadLeaderboard: withCache(_gamesService.loadLeaderboard, 2 * 60 * 1000), // 2 دقائق
  loadUserStats: withCache(_gamesService.loadUserStats, 3 * 60 * 1000), // 3 دقائق
  searchGames: withCache(_gamesService.searchGames, 5 * 60 * 1000), // 5 دقائق
  getGameCategories: withCache(_gamesService.getGameCategories, 15 * 60 * 1000), // 15 دقيقة
  // الوظائف التي لا تحتاج caching
  saveUserScore: _gamesService.saveUserScore,
  saveUserAchievement: _gamesService.saveUserAchievement,
  getUserAchievements: _gamesService.getUserAchievements
}

export const getGameCategories = gamesService.getGameCategories

// وظائف إدارة الـ cache للألعاب
export const invalidateGamesCache = () => {
  cacheInvalidate('loadGames')
  cacheInvalidate('loadGame')
  cacheInvalidate('loadLeaderboard')
  cacheInvalidate('searchGames')
  cacheInvalidate('getGameCategories')
}

export const invalidateUserStatsCache = (userId) => {
  cacheInvalidate(`loadUserStats:${userId}`)
}
