...
export const loadAdvertisements = async (filters = {}) => {
  try {
...
export const getAdStatistics = async (adId, dateRange = '7d') => {
  try {
...
export const getActiveAdsByPosition = async (position, limit = 5) => {
  try {
...
export const getAllActiveAds = async (limit = 50) => {
  try {
    const now = new Date().toISOString()
...
// البحث في الإعلانات
export const searchAdvertisements = async (query, filters = {}) => {
  try {
...
export const getPerformanceReport = async (dateRange = '30d') => {
  try {
    const daysAgo = parseInt(dateRange.replace('d', ''))
    const startDate = new Date()
    startDate.setDate(startDate.getDate() - daysAgo)
