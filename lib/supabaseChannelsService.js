...
export const loadChannels = async () => {
  try {
    ...
    if (sessionError || !session) {
      // لا توجد جلسة صالحة، سيتم استخدام المفاتيح العامة
    }
    ...
    if (error) {
      ...
      if (error.code === 'PGRST303' || (error.message && error.message.includes('JWT expired'))) {
        // انتهت صلاحية الرمز المميز، محاولة تحديث الجلسة...
        try {
          ...
          if (!refreshError && newSession) {
            ...
            if (!retryError) {
              // تم تحميل القنوات بنجاح بعد تحديث الجلسة
              return retryData || []
            }
          }
        } catch (refreshErr) {
...
// حفظ جميع القنوات في Supabase
export const saveAllChannels = async (channels) => {
  try {
    // حذف جميع القنوات الموجودة
    ...
    console.log(`تم حفظ ${data.length} قناة في Supabase`)
    return true
  } catch (error) {
    console.error('خطأ في saveAllChannels:', error)
    return false
  }
}
