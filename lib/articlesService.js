...
// تحميل جميع المقالات مع caching
const _loadArticles = async (filters = {}) => {
  try {
...
// تحميل فئات المقالات مع caching
const _loadArticleCategories = async () => {
  try {
...
// الحصول على مقال بالمعرف مع caching
const _getArticleById = async (articleId) => {
  try {
...
// تصدير الوظائف مع caching محسن
export const loadArticles = withCache(_loadArticles, 3 * 60 * 1000) // 3 دقائق
export const loadArticleCategories = withCache(_loadArticleCategories, 10 * 60 * 1000) // 10 دقائق
export const getArticleById = withCache(_getArticleById, 5 * 60 * 1000) // 5 دقائق

// وظائف إضافية لإدارة الـ cache
export const invalidateArticlesCache = () => {
  cacheInvalidate('_loadArticles')
  cacheInvalidate('_getArticleById')
}

export const invalidateCategoriesCache = () => {
  cacheInvalidate('_loadArticleCategories')
}